...
/**
 * Coppie uniche non orientate; null se input invalido o con duplicati.
 */
export function normalizeSegmentSet(segments) {
  if (!segments?.length) return [];

  const unique = new Map();
  for (const seg of segments) {
    const a = stationId(seg.from);
    const b = stationId(seg.to);
    if (a === null || b === null || a === b) return null;

    const key = segmentKey(a, b);
    if (unique.has(key)) return null;
    unique.set(key, { a: Math.min(a, b), b: Math.max(a, b) });
  }
...
/**
 * Ricostruisce un percorso continuo start→end usando tutti i segmenti scelti.
 * Rispetta l'ordine inviato se già valido; altrimenti prova a riordinarli.
 */
export function resolveSegmentsIntoPath(segments, startId, endId) {
  const start = stationId(startId);
  const end = stationId(endId);
  if (start === null || end === null || !segments?.length) return null;

  const inOrder = orientSegmentsForPath(segments, start);
  if (inOrder && inOrder[inOrder.length - 1].to === end) {
    return inOrder;
  }

  const edges = normalizeSegmentSet(segments);
  if (!edges?.length) return null;

  const pool = edges.map(edge => ({ ...edge, used: false }));
  ...
  function dfs(current, usedCount) {
    ...
    for (let i = 0; i < pool.length; i++) {
      ...
      pool[i].used = true;
      const tail = dfs(next, usedCount + 1);
      if (tail !== null) {
        return [step, ...tail];
      }
      pool[i].used = false;
